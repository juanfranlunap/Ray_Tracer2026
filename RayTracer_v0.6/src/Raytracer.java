...
public class Raytracer {
    ...
    // NEW - no more single light, the lights now live in the scene
    ...
    private Color shade(Intersection intersection, Ray ray) {
        ...
        for (int i = 0; i < lights.size(); i++) {
            ...
            // Diffuse = LC x OC x LI x (N · L) - slide 9 v0.4
            totalR += lcR * ocR * light.intensity * NdotL;
            totalG += lcG * ocG * light.intensity * NdotL;
            totalB += lcB * ocB * light.intensity * NdotL;
        }

        // we clamp values between 0 and 255 so colors dont overflow
